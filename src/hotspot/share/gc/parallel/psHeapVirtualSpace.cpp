...
void PSHeapVirtualSpace::right_shift_gen_boundary(char* new_boundary) {
  ...
  // Capture current young gen committed size
  size_t original_young_gen_committed_size = young_gen_committed_size();

  _gen_boundary = new_boundary;

  if (_young_gen_committed_high_addr < new_boundary) {
    _old_gen_committed_high_addr = _young_gen_committed_high_addr;
    size_t diff = pointer_delta(new_boundary, _young_gen_committed_high_addr, sizeof(char));
...
bool PSHeapVirtualSpace::expand_old_gen(size_t bytes) {
  ...
  if (os::commit_memory(old_high, bytes, _alignment, !ExecMem)) {
    _old_gen_committed_high_addr = new_high;
    return true;
  }
...
bool PSHeapVirtualSpace::left_expand_young_gen(size_t bytes) {
  ...
  char* new_low = old_low - bytes;
  assert(new_low >= _old_gen_committed_high_addr, "inv");

  if (_special) {
    _gen_boundary = new_low;
    return true;
  }

  if (os::commit_memory(new_low, bytes, _alignment, !ExecMem)) {
    _gen_boundary = new_low;
    return true;
  }
  ...
  log_warning(gc)("PSHeapVirtualSpace::left_expand_young_gen failed: "
                  "os::commit_memory(" PTR_FORMAT ", %zu, %zu, %d) failed.",
                  p2i(new_low), bytes, _alignment, !ExecMem);
