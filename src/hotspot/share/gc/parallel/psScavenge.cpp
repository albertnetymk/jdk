...
void PSScavenge::set_young_generation_boundary(HeapWord* v) {
  ...
  if (UseCompressedOops) {
    // Workaround when v is heap-end, i.e. old-gen-only mode.
    ParallelScavengeHeap* heap = ParallelScavengeHeap::heap();
    bool is_gen_boundary_at_heap_end = (void*) v == heap->reserved_region().end();
    _young_generation_boundary_compressed = is_gen_boundary_at_heap_end
                                          ? (uintptr_t)CompressedOops::encode_not_null(cast_to_oop(v-1)) + 1
                                          : (uintptr_t)CompressedOops::encode_not_null(cast_to_oop(v));
  }
}
