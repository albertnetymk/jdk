...
PSYoungGen::PSYoungGen(PSHeapVirtualSpace* vs, size_t initial_size) :
  _heap_vs(vs),
  _eden_space(nullptr),
  _from_space(nullptr),
  _to_space(nullptr),
...
void PSYoungGen::initialize_work() {
  ...
  size_t heap_reserved_size = heap->reserved_region().byte_size();
  // Young-gen can become zero-sized in old-gen only mode.
  size_t young_gen_min_size = 0;
  size_t young_gen_max_size = max_capacity_in_bytes();
...
void PSYoungGen::expand_to_reserved() {
  assert(_eden_space->is_empty(), "precondition");

  size_t uncommitted_size = this->uncommitted_size();
  if (uncommitted_size == 0) {
    return;
  }

  if (!_heap_vs->expand_young_gen(uncommitted_size)) {
    vm_exit_out_of_memory(uncommitted_size, OOM_MMAP_ERROR, "PSYoungGen::expand_to_reserved");
  }
}
...
void PSYoungGen::try_resize(size_t desired_eden_size, size_t desired_survivor_size) {
  ...
  if (new_gen_size < min_gen_size()) {
  ...
  } else if (new_gen_size > reserved_size()) {
    log_info(gc, ergo)("Requested sizes exceeds young-gen size (K): %zu vs %zu", new_gen_size/K, reserved_size()/K);
    ...
    if (2 * desired_survivor_size >= reserved_size()) {
      // If requested survivor size is too large
      desired_survivor_size = align_down((reserved_size() - SpaceAlignment) / 2, SpaceAlignment);
      desired_eden_size = reserved_size() - 2 * desired_survivor_size;
    } else {
...
void PSYoungGen::reinit_after_full_gc() {
  ...
  size_t survivor_size = committed_size / (SurvivorRatio + 2);
  survivor_size = align_down(survivor_size, SpaceAlignment);
...
size_t PSYoungGen::max_capacity_in_bytes() const {
  return _heap_vs->reserved_bytes() - SpaceAlignment * 2;  // to_space() is only used during scavenge
}
